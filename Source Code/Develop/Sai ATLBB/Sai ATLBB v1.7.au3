...
Global $AutoBeta=" [Thử nghiệm]"						;Chương Trình thử nghiệm
Global $AutoClass="AutoIt v3 GUI"				;Mã phân loại Chương Trình		
Global $AutoVersion="1.7"						;Phiên Bản
...
;~ 	CheckShowMap()
	...
	If $Pause Then 	GUICtrlSetState($Button_Start,$GUI_DISABLE)
	If Not $Pause Then GUICtrlSetState($Button_Start,$GUI_ENABLE)
	SetShotcutSkill()
	ShowDangerPos()
	CheckPetHP()
	ShowPetHP()
...
Func CheckShowMap()
	...
	Local $Color=PixelGetColor($CheckColorPos[0],$CheckColorPos[1])
;~ 	ToolTip($Color,$PetHPPos[0],$PetHPPos[1])
...
Func ShowAttentionAuto()
	...
	$text=$text&" - Cho nên hãy [Pause] Auto khi không đánh quái."&@LF
	$text=$text&" - Chương trình sẽ tự [Pause] khi Máu của thú bị che."&@LF
	...
	$text=$text&" - Không bị Admin Thiên Long ghét bỏ. ( O_O)"&@LF
;~ 	$text=$text&@LF
	ToolTip($text,$x,$y)
EndFunc
...
Func GetGameHandle()
	For $i=1 To 16 Step 1
		ToolTip("Bạn có "&$i&" giây để chọn Game.",$WarningPos[0],$WarningPos[1])
...
Func GetPetHPPos()
	;Kiểm tra và lấy vị trí của Game
...
Func CheckPetHP()
	If Not GetPetHPPos() Then Return -7
...
Func ActiveAutoUsePetFood()
	$Running = Not $Running
	If $Running Then WinActivate($GameHandle)
EndFunc
...
Func CreateInterface()
	...
	;Giao diện
	Global $MainGUI=GUICreate($AutoName&$AutoBeta,$MainGUISize[0],$MainGUISize[1],$MainGUIPos[0],$MainGUIPos[1],$WS_BORDER,$WS_EX_TOPMOST+$WS_EX_TOOLWINDOW)
...
;~ 	GUICtrlSetState($Button_Pause,$GUI_DISABLE)
	...
	Global $Lable_Second_Pos[2]=[$Input_DelayTime_Pos[0]+$Input_DelayTime_Size[0],$Input_DelayTime_Pos[1]]
	...
	GUICtrlSetColor($Lable_DangerPos,0x0004FF)
