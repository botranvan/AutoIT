...
;~ Biến Mô Tả Chương Trình
	...
	Global $AutoClass="AutoIt v3 GUI"				;Mã phân loại Chương Trình	
	Global $AutoVersion="1.1"						;Phiên Bản
...
Func SaveTemp($FileName = $TemFile)
...
Func MoveContent($FileName)
	$source = $TemFile
	$dest = @WorkingDir&"\"&$ListWebSite[$WebID][1]&"\"&$ListWebSite[$WebID][2]&"_1"&"\"&$FileName&".txt"
...
Func SaveContent($LN)
	Local $ConfigFile = @WorkingDir&"\"&$ListWebSite[$WebID][1]&"\"&$ListWebSite[$WebID][2]&"_1"&".ini"
	...
	While 1
		...
		Switch $ArrayLine[1]
			...
			Case "License Status"
				...
				If Not StringInStr($Line,$licensestatus) Then 
					NextURLNumber()
					WriteLog("Contractor này đã bị hũy hoặc tạm khóa.")
					WriteLog("Kết thúc với "&$URLNumber)
					WriteLog("")
					Return 0
				EndIf
