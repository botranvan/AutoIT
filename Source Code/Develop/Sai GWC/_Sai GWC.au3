...
Global $ProjectTask4I = 1			;1 - Cách hiện thị thứ nhất của trạng thái hoạt động 
...
Global $ProjectID = 2000000000		;Số lượng Project ghi nhận mỗi ngày
...
;~ 	Thực hiện công việc
	If $WebID = 2 Or $WebID = 3 Then RunningTask()
	If $WebID = 1 Then ProjectTask1()
	If $WebID = 4 Then ProjectTask4()
...
;~ 	Nút tải nhưng thông tin trong kho lên Web
	...
	GUICtrlSetState($InStore_Button,$GUI_DISABLE)
...
;~ Biến Mô Tả Chương Trình
	...
	Global $AutoVersion="2.9"						;Phiên Bản
...
;~ 	$ListWebSite[1][5]= "_keiths_market_store.html"	
	...
	$ListWebSite[4][3]= "http://www.constructionbidsource.com/dbeads/"
	$ListWebSite[4][4]= "dbe_detail.php?recordID="
	$ListWebSite[4][5]= ""
...
Func LoadData()
	...
	If $WebID = 1 Then
		...
		$ProjectID = IniRead($ConfigName,$WebID,"$ProjectID",$ProjectID)*1
	EndIf
...
;~ 	FileSetAttrib($ConfigName,"+H")
	...
	If $WebID = 1 Then
		...
		IniWrite($ConfigName,$WebID,"$ProjectID",$ProjectID)
	EndIf
EndFunc
...
Func CheckLogin($Content)
	...
	Local $NewString = StringReplace($Content,"Welcome hhb412000","")
...
;~ 	GUICtrlSetData($RunningTask_Lable,$URLNumber&": kiểm tra đăng nhập.")
;~ 	CheckLogin($Content)
	WinMove('Constructionwork','',@DesktopWidth-77,@DesktopHeight-106)
	WinClose('http://www.constructionwork.com/workers_panel.php','')
EndFunc
...
;~ 					If $Next Then $URLCount+=1
			...
			Else
				SetURL()
			EndIf
		Case $ProjectTask4I=2
			WriteLog("Bắt đầu với "&$URLNumber&" của "&$ListWebSite[$WebID][1]&" trên "&$ListWebSite[$WebID][2])
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": đang tạo URL.")
			WriteLog("Tạo URL: "&@TAB&$URL)
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": đang truy cập.")
			OpenURL()
		Case $ProjectTask4I=3
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": đang kiểm tra nội dung.")
			CheckContent4()
		Case $ProjectTask4I=4
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": đang lưu nội dung.")
			SaveTemp()
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": tắt trình duyệt.")
			CloseURL()
		Case $ProjectTask4I=5
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": lấy vùng nội dung cần.")
			FiltContent()
			MoveContent($URLNumber)
			msgbox(0,"","")
		Case $ProjectTask4I=6
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": định dạng vùng nội dung.")
			FormatContent4()
		Case $ProjectTask4I=7
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": đang lưu nội dung.")
			FileDelete($TempFile)
			SaveContent4($URLNumber)
			SaveContent4($URLNumber,$NewFile)
			GUICtrlSetData($RunningTask_Lable,$URLNumber&": Hoàn tất.")
			WriteLog("Kết thúc với "&$URLNumber)
			WriteLog("")
	EndSelect
		
	If $ProjectTask4I=7 Then 
		$ProjectTask4I=0
	EndIf
	$ProjectTask4I+=1
...
Func SetTestInfo()
	...
	$Test01&= @LF&"$ProjectTask4I: "&$ProjectTask4I
