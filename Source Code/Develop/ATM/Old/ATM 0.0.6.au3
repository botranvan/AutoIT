...
Func SetMainVar()
	...
	Global $Scan_MineI = 1				; Mã số công việc của scan 8 ô xung 1 Mine
	...
	Global $Cur_Mine[2] = [-1,-1]
	Global $Cur_Mine_Pos[2] = [-1,-1]
	Global $Mine_Need_Flag[1] = [0]



	Global $FileMineNeedFlag = "MineNeedFlag.ini"	;Lưu thông tin của Mine
...
;~ Biến Mô Tả Chương Trình
	...
	Global $AutoVersion = "0.0.6"				;Phiên Bản
...
Func Start_Play_Task()
	...
	Switch $TaskI
		Case 1 ;Kích hoạt Mine cho đến khi tìm trúng Mạch, lấy $Cur_Mine
			If Game_Scan_Random() == 0 Then
				Game_Click_Mine()
				If Game_Is_Over() Then Send("{F2}")
				If Game_Get_Open_Mine() > 7 Then $TaskI = 2
			EndIf
		Case 2 ;Tìm các con số xung quanh $Cur_Mine, lưu lại danh sách các con số
			Game_Scan_Around()
		Case 3 ;Tìm Flag cho các ô xung quanh ô hiện tại
			Mine_Find_Flag()
		Case 4
...
Func Game_Click_Mine()
	MouseClick("left",$Cur_Mine[0],$Cur_Mine[1])
...
Func Game_Scan_All($x = 0, $y = 0)
	Local $Mines_Size = Game_Get_Mode()
	Local $First_Mine = Game_Fist_Mine_Pos()
	$Cur_Mine = $First_Mine

	For $j = $y To $Mines_Size[0]-1 Step 1
	For $i = $x To $Mines_Size[1]-1 Step 1
		Game_Scan_Mine($i,$j)
	Next
	Next
EndFunc
...
Func Game_Scan_Random()
	Local $Mines_Size = Game_Get_Mode()
	$Cur_Mine_Pos[0] = Random(1,$Mines_Size[0]-1,1)
	$Cur_Mine_Pos[1] = Random(1,$Mines_Size[1]-1,1)
	Return Game_Scan_Mine($Cur_Mine_Pos[0],$Cur_Mine_Pos[1])
EndFunc

;~ Tìm xung quanh 1 Mine leesai
Func Game_Scan_Around()
	Local $Mine_Around_Pos[2] = [-1,-1]
	$Mine_Around_Pos = Mine_Get_Around($Scan_MineI)
	Local $Mine_Around[2] = [-1,-1]
	$Mine_Around = Mine_Get_Pos($Mine_Around_Pos[0],$Mine_Around_Pos[1])
	If $Mine_Around = -1 Then return "null"

	Local $Mine_Around_Number = Mine_Get_Number($Mine_Around)
	If $Mine_Around_Number > 0 Then
		_ArrayAdd($Mine_Need_Flag,$Mine_Around_Pos[0]&"x"&$Mine_Around_Pos[1]&"="&$Mine_Around_Number)
		$Mine_Need_Flag[0]+=1
	EndIf

	$Scan_MineI+=1
	If $Scan_MineI == 9 Then
		$Scan_MineI = 1
		$TaskI = 3
		Return
	EndIf

;~ 	tooltip($Mine_Around_Number,$Mine_Around[0],$Mine_Around[1])
EndFunc

;~ Quét Mine kế tiếp của Mine hiện tại
Func Game_Scan_Next_Mine()
	Local $Next[2] = [-1,-1]
	$Next[0] = $Cur_Mine_Pos[0]+1
	$Next[1] = $Cur_Mine_Pos[1]
	return Game_Scan_Mine($Next[0],$Next[1])
EndFunc

;~ Hàm quét tìm Mine tại 1 điểm
Func Game_Scan_Mine($x,$y)
	$Cur_Mine = Mine_Get_Pos($x,$y)
	If $Cur_Mine = -1 Then Return "null"

	If Not IsArray($Cur_Mine) Then Return 0
	Return Mine_Get_Number($Cur_Mine)
EndFunc

;~ Lấy vị trí của quả bom đầu tiện
Func Game_Fist_Mine_Pos()
	If Not Game_Get_CaretPos() Then return 0
...
Func Mine_Get_Pos($x,$y)
	...
	If $x < 0 Or $y < 0 or $x > $GameMode[0] Or $y > $GameMode[1] Then Return -1
	Local $First_Mine = Game_Fist_Mine_Pos()
	If Not IsArray($First_Mine) Then Return 0
...
Func Mine_Get_Number($Mine)

	Local $ColorSeach = PixelSearch($Mine[0]-$ScanSize,$Mine[1]-$ScanSize,$Mine[0]+$ScanSize,$Mine[1]+$ScanSize,$Color1,$ColorShade)
	If Not @error Then Return 1

	$ColorSeach = PixelSearch($Mine[0]-$ScanSize,$Mine[1]-$ScanSize,$Mine[0]+$ScanSize,$Mine[1]+$ScanSize,$Color2,$ColorShade)
	If Not @error Then Return 2

	$ColorSeach = PixelSearch($Mine[0]-$ScanSize,$Mine[1]-$ScanSize,$Mine[0]+$ScanSize,$Mine[1]+$ScanSize,$Color3,$ColorShade)
	If Not @error Then Return 3

	$ColorSeach = PixelSearch($Mine[0]-$ScanSize,$Mine[1]-$ScanSize,$Mine[0]+$ScanSize,$Mine[1]+$ScanSize,$Color4,$ColorShade)
	If Not @error Then Return 4

	$Mine[1]-=8
	$ColorSeach = PixelSearch($Mine[0],$Mine[1],$Mine[0]+$ScanSize,$Mine[1]+$ScanSize,$Color0)
...
Func Mine_Get_Around($Number,$Pos = 'null')
	If $Pos == 'null' Then $Pos = $Cur_Mine_Pos
	Switch $Number
		Case 1
			$Pos[0]-=1
			$Pos[1]-=1
...
;~ Lưu thông tin của một Mine
Func Mine_Save($Mine_Around_Pos,$Mine_Around_Number,$FileName)
	Local $Section[1] = [0]
	$Section = IniReadSectionNames($FileName)
	If @error Then Local $Section[1] = [0]
	$Section[0]+=1

	IniWrite($FileName,$Section[0],$Key[0],$Mine_Around_Pos[0]&"x"&$Mine_Around_Pos[1]&"="&$Mine_Around_Number)
EndFunc

;~ Tìm cờ cho những ô được đánh dấu
Func Mine_Find_Flag($FileName = 'null')
	_ArrayDisplay($Mine_Need_Flag)
	Reset_Auto()
EndFunc
...
Func SetTestInfo()
	...
	$Test01&= "$Scan_MineI: "&$Scan_MineI&@LF
	$Test01&= "$GameMode: "&$GameMode[0]&" / "&$GameMode[1]&@LF
	$Test01&= "$Cur_Mine: "&$Cur_Mine[0]&" / "&$Cur_Mine[1]&@LF
	$Test01&= "$Cur_Mine_Pos: "&$Cur_Mine_Pos[0]&" / "&$Cur_Mine_Pos[1]&@LF
