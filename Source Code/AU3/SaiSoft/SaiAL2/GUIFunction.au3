...
;~ Click nút Enchant
Func AutoEnchantClick()
	$Enchanting = Not $Enchanting
	If $Enchanting Then
		AutoEnchantSet("Stop")
		$GameHandle = WinGetHandle("[CLASS:l2UnrealWWindowsViewportWindow]")		
	Else
		AutoEnchantSet("Enchant")
	EndIf
	
EndFunc

;~ Lập giá trị cho $AutoEnchant
Func AutoEnchantGet()
	Return GUICtrlRead($AutoEnchant)
EndFunc
;~ Lập giá trị cho $AutoEnchant
Func AutoEnchantSet($NewValue = "")
	Local $Check = AutoEnchantGet()
	If $Check <> $NewValue Then GUICtrlSetData($AutoEnchant,$NewValue)
EndFunc
