...
$prop1 = _GetExtProperty($imageFile,27)
$prop2 = _GetExtProperty($imageFile,28)
If (@error) Then Exit
...
$thresholdInput = GUICtrlCreateInput ("100", 125, 25, 30, 20)
GUICtrlCreateLabel ("Width (px):", 10, 48, 110, 15)
$widthInput = GUICtrlCreateInput ($prop1, 125, 45, 30, 20)
GUICtrlCreateLabel ("Height (px):", 10, 68, 110, 15)
$heightInput = GUICtrlCreateInput ($prop2, 125, 65, 30, 20)
...
$speedInput = GUICtrlCreateInput (abs($prop1/$prop2-1), 125, 210, 25, 20)
...
For $y = 0 To ($height - 1) step $go
	For $x = 0 To ($width - 1) step $go
		$index = ($y * $width) + $x
