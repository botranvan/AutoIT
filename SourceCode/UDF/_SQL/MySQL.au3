...
Func _MySQL_InitLibrary($libmysqldll = "", $Use_EmbeddedDLL = False)
	Select
		Case __MySQL_DLLLoaded()
			; Nothing
		Case $libmysqldll <> ""
			If Not (StringRight($libmysqldll, 13) = "\libmysql.dll") And $libmysqldll <> "libmysql.dll" Then $libmysqldll &= "\libmysql.dll"
			DllOpen($libmysqldll)
			If Not __MySQL_DLLLoaded() Then ContinueCase
		Case True
			DllOpen("libmysql.dll")
			If Not __MySQL_DLLLoaded() Then
		...
		Case $Use_EmbeddedDLL = True
			Call("_" & "_MySQL_ExtractEmbeddedDLL", @TempDir & "\libmysql.dll")
			If @error Then Return SetError(3, 0, 1)
			DllOpen(@TempDir & "\libmysql.dll")
			If Not __MySQL_DLLLoaded() Then Return SetError(4, 0, 1)
	EndSelect
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_server_init", "int", 0, "ptr", 0, "ptr", 0)
...
; Description::    Closes MySQL DLL (does NOT unload it)
...
Func _MySQL_EndLibrary()
	DllCall("libmysql.dll", "none", "mysql_server_end")
...
Func _MySQL_Affected_Rows($MySQL_ptr)
	Local $row = DllCall("libmysql.dll", "uint64", "mysql_affected_rows", "ptr", $MySQL_ptr)
...
Func _MySQL_AutoCommit($MySQL_ptr, $Mode)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_autocommit", "ptr", $MySQL_ptr, "int", $Mode)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Change_User($MySQL_ptr, $User, $Pass, $Database = "")
	...
	Local $conn = DllCall("libmysql.dll", "int", "mysql_change_user", "ptr", $MySQL_ptr, "str", $User, $PWType, $Pass, $DBType, $Database)
	If @error Then Return SetError(1, 0, 1)
	Return $conn[0]
...
Func _MySQL_Character_Set_Name($MySQL_ptr)
	Local $conn = DllCall("libmysql.dll", "str", "mysql_character_set_name", "ptr", $MySQL_ptr)
...
;                  if the MySQL struct was created with _MySQLInit() without any parameter
...
Func _MySQL_Close($MySQL_ptr)
	DllCall("libmysql.dll", "none", "mysql_close", "ptr", $MySQL_ptr)
...
Func _MySQL_Commit($MySQL_ptr)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_commit", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Data_Seek($result, $offset)
	DllCall("libmysql.dll", "none", "mysql_data_seek", "ptr", $result, "uint64", $offset)
	If @error Then Return SetError(1)
...
Func _MySQL_Debug($debug)
	DllCall("libmysql.dll", "none", "mysql_debug", "str", $debug)
...
Func _MySQL_Dump_Debug_Info($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "int", "mysql_dump_debug_info", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 1)
	Return $errors[0]
...
Func _MySQL_EOF($result)
	Local $eof = DllCall("libmysql.dll", "int", "mysql_eof", "ptr", $result)
	If @error Then Return SetError(1, 0, 1)
	Return $eof[0]
EndFunc   ;==>_MySQL_EOF
...
Func _MySQL_Errno($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "uint", "mysql_errno", "ptr", $MySQL_ptr)
...
Func _MySQL_Error($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "str", "mysql_error", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
...
Func _MySQL_Fetch_Field($result)
	Local $field = DllCall("libmysql.dll", "ptr", "mysql_fetch_field", "ptr", $result)
	If @error Then Return SetError(1, 0, 0)
	$field = $field[0]
...
Func _MySQL_Fetch_Field_Direct($result, $fieldnr)
	Local $field = DllCall("libmysql.dll", "ptr", "mysql_fetch_field_direct", "ptr", $result, "uint", $fieldnr)
	If @error Then Return SetError(1, 0, 0)
	$field = $field[0]
	Return DllStructCreate($MYSQL_FIELD, $field)
...
Func _MySQL_Fetch_Fields($result, $numberOfFields = Default)
	...
	Local $fields = DllCall("libmysql.dll", "ptr", "mysql_fetch_fields", "ptr", $result)
...
Func _MySQL_Fetch_Lengths($result, $numberOfFields = Default)
	...
	Local $lengths = DllCall("libmysql.dll", "ptr", "mysql_fetch_lengths", "ptr", $result)
	If @error Then Return SetError(1, 0, 0)
...
Func _MySQL_Fetch_Row($result, $numberOfFields = Default)
	...
	Local $row = DllCall("libmysql.dll", "ptr", "mysql_fetch_row", "ptr", $result)
	If @error Then Return SetError(1, 0, 0)
...
;===============================================================================
;
Func _MySQL_Fetch_Result_StringArray($result)
...
Func _MySQL_Field_Count($result)
	Local $row = DllCall("libmysql.dll", "uint", "mysql_field_count", "ptr", $result)
	If @error Then Return SetError(1, 0, 0)
	Return $row[0]
...
Func _MySQL_Field_Seek($result, $offset)
	Local $row = DllCall("libmysql.dll", "uint", "mysql_field_seek", "ptr", $result, "uint", $offset)
	If @error Then Return SetError(1, 0, 0)
	Return $row[0]
...
Func _MySQL_Field_Tell($result)
	Local $row = DllCall("libmysql.dll", "uint", "mysql_field_tell", "ptr", $result)
	If @error Then Return SetError(1, 0, 0)
	Return $row[0]
...
Func _MySQL_Free_Result($result)
	DllCall("libmysql.dll", "none", "mysql_free_result", "ptr", $result)
	If @error Then Return SetError(1)
...
Func _MySQL_Get_Character_Set_Info($MySQL_ptr)
	...
	DllCall("libmysql.dll", "none", "mysql_get_character_set_info", "ptr", $MySQL_ptr, "ptr", DllStructGetPtr($charset))
	If @error Then Return SetError(1)
	Return $charset
...
Func _MySQL_Get_Client_Info()
	Local $errors = DllCall("libmysql.dll", "str", "mysql_get_client_info")
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
...
Func _MySQL_Get_Client_Version()
	Local $errors = DllCall("libmysql.dll", "ulong", "mysql_get_client_version")
	If @error Then Return SetError(1, 0, 0)
	Return $errors[0]
...
Func _MySQL_Get_Host_Info($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "str", "mysql_get_host_info", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
...
Func _MySQL_Get_Proto_Info($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "uint", "mysql_get_proto_info", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $errors[0]
...
Func _MySQL_Get_Server_Info($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "str", "mysql_get_server_info", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
...
Func _MySQL_Get_Server_Version($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "ulong", "mysql_get_server_version", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $errors[0]
...
Func _MySQL_Get_SSL_Cipher($MySQL_ptr, $key, $cert, $ca, $capath, $cipher)
	Local $errors = DllCall("libmysql.dll", "str", "mysql_get_ssl_cipher", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
...
Func _MySQL_Hex_String($string, $Include0x = False)
	Switch $Include0x
		Case True
			Return String(StringToBinary($string))
		Case Else
			Return Hex(StringToBinary($string))
	EndSwitch
...
Func _MySQL_Info($MySQL_ptr)
	Local $info = DllCall("libmysql.dll", "str", "mysql_info", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $info[0]
EndFunc   ;==>_MySQL_Info
...
Func _MySQL_Init($MySQL_ptr = 0)
	Local $mysql = DllCall("libmysql.dll", "ptr", "mysql_init", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $mysql[0]
EndFunc   ;==>_MySQL_Init
...
Func _MySQL_Insert_ID($MySQL_ptr)
	Local $row = DllCall("libmysql.dll", "uint64", "mysql_insert_id", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return __MySQL_ReOrderULONGLONG($row[0])
...
Func _MySQL_List_DBs($MySQL_ptr, $wild = "")
	...
	Local $query = DllCall("libmysql.dll", "int", "mysql_list_dbs", "ptr", $MySQL_ptr, $type, $wild)
	If @error Then Return SetError(1, 0, 0)
	Return $query[0]
...
Func _MySQL_List_Fields($MySQL_ptr, $table, $wild = "")
	Local $type = "str"
	If $wild = "" Then $type = "ptr"
	Local $query = DllCall("libmysql.dll", "int", "mysql_list_fields", "ptr", $MySQL_ptr, "str", $table, $type, $wild)
	If @error Then Return SetError(1, 0, 0)
	Return $query[0]
...
Func _MySQL_List_Processes($MySQL_ptr)
	Local $query = DllCall("libmysql.dll", "int", "mysql_list_processes", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $query[0]
...
Func _MySQL_List_Tables($MySQL_ptr, $wild = "")
	Local $type = "str"
	If $wild = "" Then $type = "ptr"
	Local $query = DllCall("libmysql.dll", "int", "mysql_list_tables", "ptr", $MySQL_ptr, $type, $wild)
	If @error Then Return SetError(1, 0, 0)
	Return $query[0]
...
Func _MySQL_More_Results($MySQL_ptr)
	Local $result = DllCall("libmysql.dll", "int", "mysql_more_results", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $result[0]
...
Func _MySQL_Next_Result($MySQL_ptr)
	Local $result = DllCall("libmysql.dll", "int", "mysql_next_result", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 1)
	Return $result[0]
...
Func _MySQL_Num_Fields($result)
	Local $row = DllCall("libmysql.dll", "uint", "mysql_num_fields", "ptr", $result)
	If @error Then Return SetError(1, 0, 0)
	Return $row[0]
...
Func _MySQL_Options($MySQL_ptr, $option, $arg = 0, $type = "ptr")
	...
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_options", "ptr", $MySQL_ptr, "dword", $option, $type, $arg)
	If @error Then Return SetError(2, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Ping($MySQL_ptr)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_ping", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
EndFunc   ;==>_MySQL_Ping
...
Func _MySQL_Query($MySQL_ptr, $query)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_query", "ptr", $MySQL_ptr, "str", $query)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Real_Connect($MySQL_ptr, $Host, $User, $Pass, $Database = "", $Port = 0, $unix_socket = "", $Client_Flag = 0)
	...
	Local $conn = DllCall("libmysql.dll", "ptr", "mysql_real_connect", "ptr", $MySQL_ptr, "str", $Host, "str", $User, $PWType, $Pass, $DBType, $Database, "uint", $Port, $UXSType, $unix_socket, "ulong", $Client_Flag)
	If @error Then Return SetError(1, 0, 0)
	Return $conn[0]
...
Func _MySQL_Real_Escape_String($MySQL_ptr, $From, $FromLen = Default)
	...
	Local $query = DllCall("libmysql.dll", "int", "mysql_real_escape_string", "ptr", $MySQL_ptr, "ptr", DllStructGetPtr($TO), "str", $From, "ulong", $FromLen)
	If @error Then Return SetError(1, 0, 0)
...
Func _MySQL_Real_Query($MySQL_ptr, $querystring, $querystringlength = Default)
	...
	Local $query = DllCall("libmysql.dll", "int", "mysql_real_query", "ptr", $MySQL_ptr, "str", $querystring, "ulong", $querystringlength)
	If @error Then Return SetError(1, 0, 0)
	Return $query[0]
...
Func _MySQL_Refresh($MySQL_ptr, $options)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_refresh", "ptr", $MySQL_ptr, "ulong", $options)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Reload($MySQL_ptr)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_reload", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Rollback($MySQL_ptr)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_rollback", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Row_Seek($result, $offset)
	Local $seek = DllCall("libmysql.dll", "ptr", "mysql_row_seek", "ptr", $result, "ptr", $offset)
	If @error Then Return SetError(1)
	Return $seek[0]
...
Func _MySQL_Row_Tell($result, $offset)
	Local $seek = DllCall("libmysql.dll", "ptr", "mysql_row_tell", "ptr", $result, "ptr", $offset)
	If @error Then Return SetError(1)
	Return $seek[0]
...
Func _MySQL_Select_DB($MySQL_ptr, $Database)
	Local $conn = DllCall("libmysql.dll", "int", "mysql_select_db", "ptr", $MySQL_ptr, "str", $Database)
	If @error Then Return SetError(1, 0, 1)
	Return $conn[0]
...
Func _MySQL_Set_Character_Set($MySQL_ptr, $name)
	Local $charset = DllCall("libmysql.dll", "int", "mysql_set_character_set", "ptr", $MySQL_ptr, "str", $name)
	If @error Then Return SetError(1, 0, 1)
	Return $charset[0]
...
Func _MySQL_Set_Server_Option($MySQL_ptr, $option)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_set_server_option", "ptr", $MySQL_ptr, "dword", $option)
	If @error Then Return SetError(2, 0, 1)
	Return $mysql[0]
...
Func _MySQL_Shutdown($MySQL_ptr, $shutdown_level = $SHUTDOWN_DEFAULT)
	Local $mysql = DllCall("libmysql.dll", "int", "mysql_shutdown", "ptr", $MySQL_ptr, "dword", $shutdown_level)
	If @error Then Return SetError(2, 0, 1)
	Return $mysql[0]
...
Func _MySQL_SQLState($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "str", "mysql_sqlstate", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
...
Func _MySQL_SSL_Set($MySQL_ptr, $key, $cert, $ca, $capath, $cipher)
	...
	Local $errors = DllCall("libmysql.dll", "int", "mysql_ssl_set", "ptr", $MySQL_ptr, $KeyT, $key, $CertT, $cert, $CaT, $ca, $CapathT, $capath, $CipherT, $cipher)
	If @error Then Return SetError(1, 0, 0)
	Return $errors[0]
...
Func _MySQL_Stat($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "str", "mysql_stat", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, "")
	Return $errors[0]
EndFunc   ;==>_MySQL_SQLState
...
Func _MySQL_Store_Result($MySQL_ptr)
	Local $result = DllCall("libmysql.dll", "ptr", "mysql_store_result", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $result[0]
...
Func _MySQL_Thread_Id($MySQL_ptr)
	Local $result = DllCall("libmysql.dll", "ulong", "mysql_thread_id", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $result[0]
EndFunc   ;==>_MySQL_Store_Result
...
Func _MySQL_Use_Result($MySQL_ptr)
	Local $result = DllCall("libmysql.dll", "ptr", "mysql_use_result", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $result[0]
...
Func _MySQL_Warning_Count($MySQL_ptr)
	Local $errors = DllCall("libmysql.dll", "uint", "mysql_warning_count", "ptr", $MySQL_ptr)
	If @error Then Return SetError(1, 0, 0)
	Return $errors[0]
...
;~ Global Const $simple_command(mysql, command, arg, length, skip_check) \
;~   (*(mysql)->methods->advanced_command)(mysql, command, NullS,  \
;~                                         0, arg, length, skip_check, NULL)
;~ Global Const $stmt_command(mysql, command, arg, length, stmt) \
;~   (*(mysql)->methods->advanced_command)(mysql, command, NullS,  \
;~                                         0, arg, length, 1, stmt)
