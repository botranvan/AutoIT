...
Func _BASS_Encode_Startup($sBassEncDll = "bassenc.dll")
	...
	EndIf
	;Open the DLL
	$_ghBassASIODll = DllOpen($sBassEncDll)

	;Check if the DLL was opened correctly.
	If $_ghBassASIODll <> 1 Then
		Return True
	Else
		Return False
	EndIf
...
;                  $bassenc_dll	-	Handle to opened Bassenc.dll
...
Func _BASS_Encode_GetVersion()
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_GetVersion")
...
Func _BASS_Encode_Start($handle, $cmdline, $flags, $proc = "", $user = "")
	If $proc <> "" Then
		...
		$BASSENC_ret_ = DllCall($bassenc_dll, "dword", "BASS_Encode_Start", "dword", $handle, "str", $cmdline, "dword", $flags, "ptr", DllCallbackGetPtr($dc), "ptr", DllStructGetPtr($dsUser))
	Else
		$BASSENC_ret_ = DllCall($bassenc_dll, "dword", "BASS_Encode_Start", "dword", $handle, "str", $cmdline, "dword", $flags, "ptr", 0, "ptr", 0)
	EndIf
...
Func _BASS_Encode_Stop($handle)
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_Stop", "dword", $handle)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_IsActive($handle)
	$BASSENC_ret_ = DllCall($bassenc_dll, "dword", "BASS_Encode_IsActive", "dword", $handle)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_Write($handle, $buffer, $length)
	...
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_Write", "dword", $handle, "ptr", DllStructGetPtr($ds_buffer), "DWORD", $length)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_SetNotify($handle, $proc, $user)
	...
	$dsUser = DllStructCreate("char[255]")
	DllStructSetData($dsUser, 1, $user)
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_SetNotify", "dword", $handle, "ptr", DllCallbackGetPtr($dc), "ptr", DllStructGetPtr($dsUser))
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_GetCount($handle, $count)
	$BASSENC_ret_ = DllCall($bassenc_dll, "dword", "BASS_Encode_GetCount", "dword", $handle, "dword", $count)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_SetChannel($handle, $channel)
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_SetChannel", "dword", $handle, "dword", $channel)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_GetChannel($handle)
	$BASSENC_ret_ = DllCall($bassenc_dll, "dword", "BASS_Encode_GetChannel", "dword", $handle)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_CastInit($handle, $server, $pass, $content, $name, $url, $genre, $desc, $headers, $bitrate, $pub)
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_CastInit", "dword", $handle, "str", $server, "str", $pass, "str", $content, "str", $name, "str", $url, "str", $genre, "str", $desc, "str", $headers, "dword", $bitrate, "int", $pub)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_CastSetTitle($handle, $title, $url)
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_CastSetTitle", "dword", $handle, "str", $title, "str", $url)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
...
Func _BASS_Encode_CastGetStats($handle, $stype, $pass)
	$BASSENC_ret_ = DllCall($bassenc_dll, "int", "BASS_Encode_CastGetStats", "dword", $handle, "dword", $stype, "str", $pass)
	$error = _BASS_ErrorGetCode($bass_dll)
	If $error <> 0 Then
		Return SetError($error, "", 0)
	Else
