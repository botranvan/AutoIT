...
If Not $ArrayInStore[0] Then ReLoadInStore()
...
;~ 	Nếu không bơm thì không hiển thị cái này nữa và xóa đi
		...
		If GUICtrlRead($RunningTask_Lable)<>"" then GUICtrlSetData($RunningTask_Lable,"")
		$RunningI=1
		Return False
	EndIf
...
Func UpLoadNewGet()
	...
	For $i=1 To $ArrayNewGet[0] Step +1
		RunningCheck("Đang tạo gói SQL "&GetPercent($ArrayNewGet[0],$i))
...
Func CheckInStoreLable()
	...
	Else
		If $Check<>$InStore_LableT2 Then GUICtrlSetData($InStore_Lable,$InStore_LableT2)
	EndIf
EndFunc
...
Func WriteLog($Line,$Time=1)
	...
	$DLine&=$Line
	$Log = FileOpen("log\"&@MDAY&"."&@MON&"."&@YEAR&".txt",1+32)
...
;~ 	Nơi hiển thị số lượng trong kho
	...
	GUICtrlSetOnEvent($InStore_Lable,"StartWithInStore")
...
;~ 	Nơi hiển thị kết quả vừa lấy được
	...
	GUICtrlSetOnEvent($NewGet_Lable,"StartWithNewGet")
...
;~ Biến Mô Tả Chương Trình
	...
	Global $AutoVersion="1.5"						;Phiên Bản
...
;~ Biến ngôn ngữ
	...
	Global $Start_Button_Tip="Bắt đầu lấy thông tin từ Web xuống"
	...
	Global $InStore_Button_Tip="Cập nhật lại số lượng Thông Tin hiện có trong kho"
...
;~ 	$ArrayInStore[0] = IniRead($ConfigName,$WebID,"$ArrayInStore[0]",$ArrayInStore[0])*1
EndFunc
...
;~ 	If IsArray($ArrayInStore) Then IniWrite($ConfigName,$WebID,"$ArrayInStore[0]",$ArrayInStore[0])
EndFunc
...
;~ Dùng số lớn nhất trong NewGet để làm số bắt đầu
Func StartWithNewGet()
	SetStartNumInput($ArrayNewGet[$ArrayNewGet[0]])
EndFunc

;~ Dùng số lớn nhất trong NewGet để làm số bắt đầu
Func StartWithInStore()
	SetStartNumInput($ArrayInStore[$ArrayInStore[0]])
EndFunc
...
Func FormatContent3()
	WriteLog("Kiểm duyệt nội dung cho "&$ListWebSite[$WebID][1]&" của "&$ListWebSite[$WebID][2])
