...
;~ Biến Mô Tả Chương Trình
	...
	Global $AutoVersion = "0.0.5"				;Phiên Bản
	...
	Switch $TaskI
		Case 1
			If Game_Scan_Random() == 0 Then
				...
				If Game_Get_Open_Mine() > 1 Then $TaskI = 2
			EndIf
		Case 2
;~ 			Game_Scan_All()
			Send("{F2}")
...
Func Game_Get_Mode()
	Return Game_Read_Memory($GameModeAddress)
EndFunc
...
Func Game_Scan_Random()
	...
	Return Game_Scan_Mine($RandomX,$RandomY)
EndFunc

;~ Hàm quét tìm Mine tại 1 điểm leesai
