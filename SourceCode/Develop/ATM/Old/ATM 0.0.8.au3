...
;~ Biến Mô Tả Chương Trình
	...
	Global $AutoVersion = "0.0.8"				;Phiên Bản
...
Func Game_Click_Mine($x,$y,$mode = "left")
	...
	Switch $mode
		Case $mode = 'tooltip'
			...
			tooltip($Number,$Pos[0]+4,$Pos[1]+4)
		Case $mode = 'move'
...
Func Start_Play_Task()
	...
	Switch $TaskI
		...
		Case 3
			...
			If $Pos[0] <> -1 AND ($Number == 0 OR $Number == "c") Then
				If ($Number == 0) Then
...
;~ 				Game_Click_Mine($Pos[0],$Pos[1],'tooltip')
			...
			If $Mine_AroundI >= 9 Then
				If $Space_Is_Mine[0] == $Cur_Space_Number AND $Cur_Space_Number <> 0 Then Space_Set_Flag($Space_Is_Mine)
				If $Space_Is_Flag == $Cur_Space_Number AND $Space_Is_Flag > 0 Then Space_Open($Space_Is_Mine)
			...
			EndIf
		Case 4
			If WinExists("[CLASS:#32770]","You have the fastest") Then
				Sleep(72)
				Send("72ls.net",1)
				Send("{ENTER}")
				Send("{ENTER}")
...
Func Space_Set_Flag($Space_Is_Mine)
	For $i = 1 To $Space_Is_Mine[0] Step 1
		...
		If $Pos[0] <> 3 Then Game_Click_Mine($Pos[1],$Pos[2],'right')
	Next
EndFunc
